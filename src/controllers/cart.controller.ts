...
const addToCart = async (req: Request, res: Response) => {
  ...
  if (!cart) cart = await cartService.createCart(targetUser);

  // Validate book id
  const { bookId } = req.params;
  const isValid = validateId(bookId);
  if (!isValid) throw new ApiError('Invalid book ID format', 400);
  ...
  // Create cart item
  const cartItem = cartService.createCartItem(book);

  const success = await cartService.addToCart(user._id, cartItem);
  if (!success) throw new ApiError('Something went wrong', 500);
...
const getCart = async (req: Request, res: Response) => {
  ...
  if (!isValidId) throw new ApiError('Invalid cart ID format', 400);
...
const updateCart = async (req: Request, res: Response) => {
  ...
  const isValidId = validateId(user?._id);
  if (!isValidId) throw new ApiError('Invalid user ID format', 400);

  const cart = await cartService.getCart(user._id);
  if (!cart) throw new ApiError('Cart not found', 404);

  const updatedCart = await cartService.updateItem(cart, bookId, quantity);
  if (!updatedCart) throw new ApiError('Something went wrong', 500);
...
const removeFromCart = async (req: Request, res: Response) => {
  ...
  if (!(result.modifiedCount > 0))
    res.status(200).json({ message: 'No matching book found in the cart' });

  res.status(200).json({ message: 'Item removed successfully' });
};

// Get similar books to items in cart
